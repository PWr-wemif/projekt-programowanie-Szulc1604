...
struct SecondView: View {
    ...
    func arrowSymbol(_ isUp: Bool) -> some View {
                            ...
                            ForEach(selectedSongs, id: \.name) { selectedSong in
                                HStack {
                                    ...
                                    Text(selectedSong.length)
                                        .foregroundColor(TrackStyle.albumColor2(selectedSong.album))
                                        .overlay(arrowSymbol(selectedSong.length < randomSong?.length ?? "").offset(x: -20), alignment: .leading)
                                    Spacer()
                                    Text("\(selectedSong.number)")
                                        .foregroundColor(TrackStyle.albumColor2(selectedSong.album))
                                        .overlay(arrowSymbol(selectedSong.number < randomSong?.number ?? 0).offset(x: -20), alignment: .leading)
                                    Spacer()
                                    Text(selectedSong.feat)
                                        .foregroundColor(TrackStyle.albumColor2(selectedSong.album))
                                        .overlay(
                                            selectedSong.feat == randomSong?.feat ?
